...
export async function getWeddingPictures(req: Request, env: any) {
	try {
		const url = new URL(req.url);
		const searchParams = url.searchParams;
		...
		const data = await worksService.getWeddingPictures(env, Number(page), String(category), Number(subMenu), Number(limit));
